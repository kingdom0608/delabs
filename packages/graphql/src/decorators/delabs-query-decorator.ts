...
import { Query as nestQueryDecorator } from '@nestjs/graphql';
import { DocumentBuilder } from '../builders';
import { DelabsQueryOptions } from '../interfaces';
 ...
 * @constructor
 */
export function DelabsQuery(
  nameOrType: ReturnTypeFunc,
  options: DelabsQueryOptions
): MethodDecorator {
  ...
  return (target, key?: string, descriptor?: any) => {
    ...
    /** nest/graphql @Query 데코레이터에 추가 */
    nestQueryDecorator(nameOrType, options)(target, key, descriptor);
  };
}
